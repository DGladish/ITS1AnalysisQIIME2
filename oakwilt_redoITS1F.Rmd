...
Run the following code in the linux command line to run the FUNGuild script:
# python Guilds_v1.1.py -otu /isilon/cfia-ottawa-fallowfield/users/girouxeml/gladishd/pirl_working/oakwilt_redo/otuTax_tableOW_redo_NoTaxfill.tsv -db fungi -m -u
...
FUNG_Testdir <- paste(sharedPathAn, "FUNGuild_Test", sep = "/")

prefix <- "FUNGuild"
cmd <- paste("conda activate python3.6 && ",
             " python ", FUNGScriptdir,
             " -otu ", paste(FUNG_Testdir, "otuTax_table.tsv", sep = "/"),
             " -db fungi",
             " -m ", " -u ",
             " && conda deactivate ", sep = "")
suffix <- ".sub"; cat(bashDirections); MakeQsubs(cmd, prefix, suffix)

### ^^ This code does not work, just an attempt at running the FUNGuild script. I have yet to figure out a way to run the FUNGuild script from an R script. To actually run it, see lines 422-435 above.
...
write.table(metatempdiv, file=paste(diversityPath, "OW2021redo_ITS1F.meta.div.tsv", sep = "/"),
              append    = FALSE,
              sep       = "\t",
              row.names = FALSE,
              quote=FALSE)
