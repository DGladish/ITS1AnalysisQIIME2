...
# ^^^ above for command does not work, reports "Error: object 'cmd' not found". Try the following:
...
**Note:** For the ITS2 region, itsxpress does not recognise the ITS2 amplicon regions and nothing is returned - no OTU table for ITS2 can be generated using this code (for now). An solution is currently being saught after to resolve this issue.  
```{r}
...
```
This is the code that was used to try and use ITSxpress on the reverse reads to try and extract the ITS2 region. Note that this doesn't work as of now. The result after running this code is simply 93 empty .fastq files.
```{r}
...
### ^^ this code has been giving me issues & does not work yet. I have not yet figured out how to run the FUNGuild script through RStudio.
```
...
Generate abundance bar graphs (might not be necessary. Guillaume wants top 100, but this would make the figure visually interpretable, as there are too many like colours in the bar graph. Perhaps just use the list in the excel file that is created later on in this script.
```{r}
...
## the resultant excel file demonstrates abundance of each species in order, for each sample that contains C. fagacaerum.
